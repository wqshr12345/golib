...
func (c *RtcCompressor) Compress(src []byte) []byte { //, testMeta *[]common.EventWrapperMeta
	...
	//c.perf.AddDtTime("all", totalDeserTime)
	// // runtime.GOMAXPROCS(8)
	// go func() {
	// 	for start < total {
	// 		totalEvents += 1
	// 		// 1. 反序列化数据
	// 		offset, wrapper, _ := c.deserialize(src[start:])
	...
	// 	c.columnCompress(wrapper)
	// 	columnCompressTime := time.Since(startTime)
	// 	totalcolumnCompressTime += columnCompressTime
	// }
	...
	// fmt.Println("deserialize time total", totalDeserTime)
	// fmt.Println("column compress time total", totalcolumnCompressTime)
	// fmt.Println("finalize time total", totalFinalizeTime)
	// fmt.Println("finalize subcmpr time total", common.CompressTime)
	// fmt.Println("finalize subcluster time total", totalFinalizeTime-common.CompressTime)
	// common.CompressTime = 0
	//c.perf.Du//mpPerf()
	return out
}

// TODO(wangqian):加入多线程的东西......
// func (c *RtcCompressor) deserializeLen(src []byte) (offset int, wrapper event.EventWrapper, err error) {
// 	// 1. 构造event wrapper
// 	// compress body len.
