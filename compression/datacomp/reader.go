...
func NewReader(r io.Reader, stat *statistics.TcpStatistic) *Reader {
	return &Reader{
		inR:   r,
		pkgID: 0,
		stat:  stat,
...
func (r *Reader) fill() error {
	...
	compressType := iBuf[0]
	r.stat.FetchReadCmprTimes(int(compressType))

	dataLen := int(binary.LittleEndian.Uint32(iBuf[1:5]))
	rawDataLen := int(binary.LittleEndian.Uint32(iBuf[5:]))
	...
	r.dcmpr = compression_reader_writer.NewDecompressor(compressType)

	// mid2Ts := time.Now().UnixNano()

	// Note(wangqian): 哥们儿真的服了。有的接口要求传的buflen为0，有的要求len和cap一样
	if compressType == common2.CompressTypeLz4 || compressType == common2.CompressTypeFlate || compressType == common2.CompressTypeBrotli {
