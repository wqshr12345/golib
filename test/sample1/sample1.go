...
func compressAndMeasure(data []byte, compressor common.Compressor) (float64, float64) {
	...
	// duration := userTime.Milliseconds()
	duration := userTime.Seconds()
	// duration := time.Since(start).Seconds()
	if duration == 0 {
		fmt.Println("startRunage", startRusage)
		fmt.Println("endRusage", endRusage)
		fmt.Println("startTID", startTID)
		fmt.Println("endTID", endTID)
		// panic("inf")
	}

	compressionGain := float64(len(data)) / float64(len(compressedData))
	compressionBandwidth := float64(len(compressedData)) / (1024 * 1024) / float64(duration) // MB/s
...
func main() {
	...
	names := []string{"snappy", "lz4", "zstd", "lzo", "gzip", "flate"}
	cmprTypes := []byte{common.SNAPPY, common.LZ4, common.ZSTD, common.LZO, common.GZIP, common.FLATE}
	chunkSize := 200 * 1024 * 1024 // 100 MB
	...
	for _, file := range files {
		...
		for startIndex := 0; startIndex < len(fileData); startIndex += chunkSize {
			...
			for i, name := range names {
				...
				// 	Corrects[i]++
				// }
				fmt.Printf("%s - Full Gain: %.2f, Full Bandwidth: %.2f ", name, fullGain, fullBandwidth)
			}
			numSamples++
		}
