...
cpuUsage_values=("1")
# rate与balance绑定
rate_values=("50*1024*1024")
balance_values=("15*1024*1024")
# rate_values=("2*1024*1024" "4*1024*1024" "8*1024*1024" "16*1024*1024" "32*1024*1024" "64*1024*1024" "128*1024*1024" "256*1024*1024")
# balance_values=("0*1024*1024" "0*1024*1024" "0*1024*1024" "0*1024*1024" "0*1024*1024" "0*1024*1024" "0*1024*1024" "0*1024*1024")
...
type_name_values=("ours") #ours zstd lz4 snappy
...
limit_values=("50*1024" "50*1024" "50*1024" "50*1024" "50*1024" "50*1024" "50*1024")
epoch_values=("50") # "20" "30" "40" "50" "100000000"
...
#full_file_name_values=("/home/wq/sql/Shuffleaa" "/home/wq/sql/Shuffleab" "/home/wq/sql/Shuffleac" "/home/wq/sql/Shufflead" "/home/wq/sql/Shuffleae" "/home/wq/sql/Shuffleaf" "/home/wq/sql/Shuffleag" "/home/wq/sql/Shuffleah" "/home/wq/sql/Shuffleai" "/home/wq/sql/Shuffleaj" "/home/wq/sql/Shuffleak") #"/home/wq/parsecsv/sql/Common1"
...
# full_file_name_values=("/home/wq/sql/Shuffleak")
              ...
              if [ $full = "true" ] ; then
                ...
                do
                  ...
                  if [ $type_name = "oneBest" ] ; then
                    ...
                    timeout 800 ./test -cpuUsage $cpuUsage -bufferSize $buffer_size -packageSize $package_size -limitThreshold ${limit} -rate $rate -balance $balance -typeName $type_name -epochThreshold $epoch -fileName $file_name -obName $ob_name -isFull=$full #>$output_file
                    ...
                    mb_name="wq"
                    echo "start $output_file first"
                    timeout 800 ./test -cpuUsage $cpuUsage -bufferSize $buffer_size -packageSize $package_size -limitThreshold ${limit} -rate $rate -balance $balance -typeName $type_name  -epochThreshold $epoch -fileName $file_name -mbName $mb_name -isFull=$full #>$output_file
                    echo "finished $output_file first"
                    # 第二轮，使用最优
                    ...
                    echo "start $output_file"
                    timeout 800 ./test -cpuUsage $cpuUsage -bufferSize $buffer_size -packageSize $package_size -limitThreshold ${limit} -rate $rate -balance $balance -typeName $type_name  -epochThreshold $epoch -fileName $file_name -isFull=$full #>$output_file
