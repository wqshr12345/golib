...
// 和test_all.go的实现基本一样，只不过需要去除掉波动带宽的影响
...
func NewMockWriter(w io.WriteCloser) *MockWriter {
	return &MockWriter{
		w:     w,
...
func main() {
	...
	typeName = "zstd"
	// fileName = "/home/wq/parsecsv/sql/Hatred_1.sql"
	fileName = "/home/wq/binlog/binlog001-041.txt"
	obName = "wq"
	mbName = "wq"
	epochThreshold = 10
	mbName = "/home/wq/golib/test/incr/multiBest/max_type/1_6*1024*1024*1024_10*1024*1024_75*1024*1024_0*1024*1024_50.2txt.maxtype"
	...
	conn, err := net.Dial("tcp", "101.37.162.252:5001")
	if err != nil {
	...
	rate = 10 * 1024 * 1204
	Initer := adaptive.NewIniter(NewMockWriter(conn), int(bufferSize), int(packageSize), cpuUsage, obBest, mbBest, hyBest, float64(rate), int64(limitThreshold), int64(epochThreshold), false)
	//Initer := adaptive.NewIniter(limit.NewRateLimiter(conn, float64(rate), float64(balance), 1, 3, int64(limitThreshold)), int(bufferSize), int(packageSize), cpuUsage, obBest, mbBest, hyBest, float64(rate), int64(limitThreshold))
